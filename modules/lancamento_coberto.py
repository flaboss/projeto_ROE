...
def lancamento_coberto_acoes_em_custodia(df):
    ...
    avg_price = avg_price[avg_price.available == True]  # noqa: E712
    tickers_avg_cost = {}
    ...
    l_coberto_carteira["retorno_total"] = l_coberto_carteira["op_vlr"] + l_coberto_carteira["op_strike"]
...
def lancamento_coberto_estrategia_OTM(df):
    ...
    """
    l_coberto_OTM = df[["opcao", "num_negoc", "acao", "acao_vlr", "op_strike", "op_vlr", "op_venc"]][
        (df.tipo == "CALL") & (df["op_venc"] > today) & (df["num_negoc"] >= configs["NUM_NEGOC_MIN"]["value"])
    ]
...
def lancamento_coberto_custo_final(df):
    ...
    """
    l_coberto = df[["opcao", "num_negoc", "acao", "acao_vlr", "op_strike", "op_vlr", "op_venc"]][
        (df.tipo == "CALL") & (df["op_venc"] > today) & (df["num_negoc"] >= configs["NUM_NEGOC_MIN"]["value"])
    ]
