...
future_date = (dateutil.parser.parse(today) + datetime.timedelta(days=configs["NUM_MESES"]["value"] * 30)).strftime(
    "%Y-%m-%d"
)
...
try:
    ...
    ticker_list = ticker_df[ticker_df.process > 0]["ticker"].to_list()
    ...
    try:
        ...
        df_cap_garantido_strikes = options_df[
            (
                ...
                >= options_df.acao_vlr - (options_df.acao_vlr * PERC_STRIKE_DIF_PARA_CAPITAL_GARANTIDO)
            )
        ...
        df_to_process = df_cap_garantido[
            df_cap_garantido['acao'].isin(get_tickers_to_be_processed('process_capital_garantido'))
        ]
...
if deciders["LANC_COBERTO_ACOES_CARTEIRA"]["value"] is True:
    try:
        ...
        send_push_notification(
            "Estratégia de opções", "Falha ao executar estratégia de lancamento coberto para acoes em custodia."
        )
...
if deciders["LANC_COBERTO_CUSTO_FINAL"]["value"] is True:
    try:
        ...
        df_to_process = options_df[
            options_df['acao'].isin(get_tickers_to_be_processed('process_l_coberto_custo_final'))
        ]
        ...
        send_push_notification(
            "Estratégia de opções", "Falha ao executar estratégia de lancamento coberto (custo final)."
        )
